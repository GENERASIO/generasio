...
$('.search a, .tg-btn-1, .side-toggle-icon, .mobile-nav-toggler, .dropdown-btn').on('click', () => new Audio(base_url+'/mykd/assets/audio/click.wav').play());
$('.search__close, .offCanvas__toggle, .offCanvas__overlay, .close-btn').on('click', () => new Audio(base_url+'/mykd/assets/audio/remove.wav').play());
$('.about__tab-wrap ul button').on('click', () => new Audio(base_url+'/mykd/assets/audio/tab.mp3').play());
