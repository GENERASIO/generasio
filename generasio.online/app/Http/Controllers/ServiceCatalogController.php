...
{
    public static $pageTitle = 'Service Catalog';
    // public static $pageDescription = 'Service Catalog Description';
    // public static $modelName = 'App\Models\ServiceCatalog';
    public static $permissionName = 'service-catalog';
    public static $folderPath = 'service-catalog';
    public static $routePath = 'service-catalogs';
    ...
    public function index(Request $req)
    {
        if ($req->ajax()) {
            $user = Auth::user();
            return Datatables::of(ServiceCatalog::query())->addIndexColumn()->make(true);
        }
    ...
    public function create()
    {
        ...
        return view(self::$folderPath.'.create', compact('serviceCatalog', 'pageTitle', 'routePath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('file')) {
            ...
            $request->file->move(public_path('uploads/game'), $fileName);

            $req['file'] = $fileName;
            $req['file_dir'] = 'uploads/game';
        }
        if ($request->file('guide')) {
            $name = $request->file('guide')->getClientOriginalName();
            ...
            $fileName = date('Y_m_d_').rand(1,99).rand(1,99).'_'.$name;  
            $request->guide->move(public_path('uploads/guide'), $fileName);

            $req['guide'] = $fileName;
            $req['guide_dir'] = 'uploads/guide';
        }

        $serviceCatalog = ServiceCatalog::create($req);

        // task accumulation
        User::find(Auth::user()->id)->update([
            'task_done' => (Auth::user()->task_done + 1)
        ]);

        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' menambahkan ' . self::$pageTitle . ' ' . $serviceCatalog->id,
    ...
    {
        $serviceCatalog = ServiceCatalog::find($id);
        $user = Auth::user();

        $pageTitle = self::$pageTitle;
        $routePath = self::$routePath;

        return view(self::$folderPath.'.show', compact('serviceCatalog', 'pageTitle', 'routePath'));
    }
    ...
    public function edit($id)
    {
        ...
        return view(self::$folderPath.'.edit', compact('serviceCatalog', 'pageTitle', 'routePath'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' mengubah ' . self::$pageTitle . ' ' . $serviceCatalog->id,
            'icon' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function destroy(Request $req, $id)
    {
        ...
        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' menghapus ' . self::$pageTitle . ' ' . $serviceCatalog->id,
            'icon' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
