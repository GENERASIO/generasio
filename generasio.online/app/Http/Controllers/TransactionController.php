...
{
    public static $pageTitle = 'Transaction';
    // public static $pageDescription = 'Transaction Description';
    // public static $modelName = 'App\Models\Transaction';
    public static $permissionName = 'transaction';
    public static $folderPath = 'transaction';
    public static $routePath = 'transactions';
    ...
    public function index(Request $req)
    {
        if ($req->ajax()) {
            $user = Auth::user();
            return Datatables::of(Transaction::with('catalog', 'product')->get())->addIndexColumn()->make(true);
        }
    ...
    public function create()
    {
        ...
        return view(self::$folderPath.'.create', compact('transaction', 'pageTitle', 'routePath'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' menambahkan ' . self::$pageTitle . ' ' . $transaction->id,
    ...
    {
        $transaction = Transaction::with('catalog', 'product')->where('id', $id)->first();
        $user = Auth::user();

        $pageTitle = self::$pageTitle;
        $routePath = self::$routePath;

        return view(self::$folderPath.'.show', compact('transaction', 'pageTitle', 'routePath'));
    }
    ...
    public function edit($id)
    {
        ...
        return view(self::$folderPath.'.edit', compact('transaction', 'pageTitle', 'routePath'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' mengubah ' . self::$pageTitle . ' ' . $transaction->id,
            'icon' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function destroy(Request $req, $id)
    {
        ...
        // logs
        ActivityLog::create([
            'name' => Auth::user()->name . ' menghapus ' . self::$pageTitle . ' ' . $transaction->id,
            'icon' => '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
