...
<body>
                                ...
                                <div class="tgmenu__navbar-wrap tgmenu__main-menu d-none d-xl-flex">
                                    <ul class="navigation">
                                        @foreach ($menus as $menu)
                                            <li class="{{ ((!empty($menu->submenu) && count($menu->submenu) > 0) ? 'menu-item-has-children' : '') }} {{ url($menu->url) == url()->current() . '/ ' ? 'active' : '' }} {{ request()->is($menu->url) ? 'active' : '' }} {{ request()->is('/'.$menu->url)  ? 'active' : '' }}"><a href="{{ url($menu->url) }}">{{ $menu->title }}</a>
    ...
    <script>
        ...
        if(randomNumber == 1 || randomNumber == 2 || randomNumber == 3){
            ...
            $.ajax({
                url: `{{ route('api.update-service-product') }}`,
                data: {game: "Mobile Legends A"},
                dataType: 'json',
                success: function(result) {
        ...
        if(randomNumber == 9 || randomNumber == 10){
            ...
            $.ajax({
                url: `{{ route('api.update-service-product') }}`,
                data: {game: "Mobile Legends Promo"},
                dataType: 'json',
                success: function(result) {
